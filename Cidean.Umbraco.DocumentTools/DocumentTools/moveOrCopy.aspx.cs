...
namespace Cidean.Umb.DocumentTools
{
    ...
    public partial class moveOrCopy : UmbracoEnsuredPage
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (app == "settings")
                {
                    ...
                    DocumentType dt = new DocumentType(int.Parse(helper.Request("id")));

                    //Load master types... 
                    masterType.Attributes.Add("style", "width: 350px;");
                    masterType.Items.Add(new ListItem(ui.Text("none") + "...", "0"));
                    foreach (umbraco.cms.businesslogic.web.DocumentType docT in umbraco.cms.businesslogic.web.DocumentType.GetAllAsList())
                    {
                ...
                else
                {
                    ...
                    {
                        if (int.Parse(s) > 0)
                            currentPath += "/" + new umbraco.cms.businesslogic.web.Document(int.Parse(s)).Text;
                    }
        ...
        private void handleChildNodes(umbraco.cms.businesslogic.web.Document d)
        {
            foreach (umbraco.cms.businesslogic.web.Document cd in d.Children)
            {
                if (cd.Published)
                {
        ...
        private void HandleDocumentTypeCopy()
        {
            ...
            DocumentType eDt = new DocumentType(int.Parse(helper.Request("id")));

            //Documentype exists.. create new doc type... 
            if (eDt != null)
            {
                string Alias = rename.Text;
                umbraco.cms.businesslogic.web.DocumentType dt = umbraco.cms.businesslogic.web.DocumentType.MakeNew(base.getUser(), Alias.Replace("'", "''"));
                ...
                foreach (DocumentType.TabI tab in eDt.getVirtualTabs)
                {
                    int tId = dt.AddVirtualTab(tab.Caption);
                    oldNewTabIds.Add(tab.Id, tId);
                }

                foreach (PropertyType pt in eDt.PropertyTypes)
                {

                    PropertyType nPt = PropertyType.MakeNew(pt.DataTypeDefinition, dt, pt.Name, pt.Alias);
                ...
                //Page.ClientScript.RegisterClientScriptBlock(this.GetType(), "refreshTree", "<script>\ntop.refreshTree(false, true);\ntop.closeModal();</script>");
        ...
        public void HandleMoveOrCopy(object sender, EventArgs e)
        {
            if (helper.Request("app") == "settings")
                ...
                if (OnlyChildren.Checked)
                    HandleDocumentMoveOrCopyChildren();
                else
        ...
        private void HandleDocumentMoveOrCopy()
        {
            if (helper.Request("copyTo") != "" && helper.Request("id") != "")
            {
                ...
                umbraco.cms.businesslogic.Content currentNode = new umbraco.cms.businesslogic.Content(int.Parse(helper.Request("id")));
                int oldParent = -1;
                if (currentNode.Level > 1)
                    oldParent = currentNode.Parent.Id;
                umbraco.cms.businesslogic.Content newNode = new umbraco.cms.businesslogic.Content(int.Parse(helper.Request("copyTo")));
                ...
                else
                {
                    ...
                    if (!nodeAllowed)
                    {
                        ...
                        feedback.Text = "Child Nodes not allowed.";
                ...
                if (nodeAllowed)
                {
                    ...
                    if (helper.Request("mode") == "cut")
                    {
                        ...
                        umbraco.cms.businesslogic.web.Document d = new umbraco.cms.businesslogic.web.Document(int.Parse(helper.Request("id")));
                        ...
                        feedback.Text = ui.Text("moveOrCopy", "moveDone", nodes, base.getUser()) + "</p><p><a href='#' onClick='top.closeModal()'>" + ui.Text("closeThisWindow") + "</a>";
                        feedback.type = umbraco.uicontrols.Feedback.feedbacktype.success;
                    ...
                    else
                    {
                        ...
                        feedback.Text = ui.Text("moveOrCopy", "copyDone", nodes, base.getUser()) + "</p><p><a href='#' onClick='top.closeModal()'>" + ui.Text("closeThisWindow") + "</a>";
                        feedback.type = umbraco.uicontrols.Feedback.feedbacktype.success;
        ...
        private void HandleDocumentMoveOrCopyChildren()
        {
            if (helper.Request("copyTo") != "" && helper.Request("id") != "")
            {
                // Check if the current node is allowed at new position
                ...
                else
                {
                    ...
                    foreach (umbraco.cms.businesslogic.CMSNode childNode in currentNode.Children)
                    {

                        bool childAllowed = false;
                        foreach (int i in newNode.ContentType.AllowedChildContentTypeIDs)
                            if (i == (new umbraco.cms.businesslogic.Content(childNode.Id)).ContentType.Id)
                            {
                                childAllowed = true;
                                break;
                            }
                ...
                if (nodeAllowed)
                {
                    ...
                    if (helper.Request("mode") == "cut")
                    {
                        ...
                        {
                            childDocument.Move(int.Parse(helper.Request("copyTo")));
                            if (childDocument.Published)
                            {
                                childDocument.Publish(new umbraco.BusinessLogic.User(0));
                                umbraco.library.PublishSingleNode(childDocument.Id);

                                if (childDocument.HasChildren)
                                {
                                    handleChildNodes(childDocument);
                                }
                            }
                            childDocument.Save();
                        }
                    ...
                    else
                    {
                        ...
                        foreach (Document childDocument in d.Children)
                        {
                            childDocument.Copy(int.Parse(helper.Request("copyTo")), this.getUser(), RelateDocuments.Checked);
                        }
